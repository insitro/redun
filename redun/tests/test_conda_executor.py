...
def main(count_file: File, conda: str, pip: str) -> List[int]:
    return [
        task1.options(conda=conda, pip=pip)(1, 2, count_file),
        task1.options(conda=conda, pip=pip)(2, 2, count_file),
    ]


def create_conda_env_file():
    with tempfile.NamedTemporaryFile(
        mode="w", prefix="redun_test_env", suffix=".yml", delete=False
    ) as f:
        f.write(
            dedent(
                f"""
...
def test_conda_executor() -> None:
    ...
    [(pid1, pyv1), (pid2, pyv2)] = scheduler.run(
        main(count_file, conda=env_file, pip=str(dev_requirements))
    )
    assert pid1 != pid2
    assert pyv1 == pyv2
