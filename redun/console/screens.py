...
class SearchScreen(RedunScreen):
    ...
    async def load_results(self) -> None:
        if format_args(self.parser, self.args) == []:
            self.results = []  # ty: ignore[invalid-assignment]
            return
        ...
        self.results = list(query.page(self.page - 1, self.page_size))  # ty: ignore[invalid-assignment]
...
class ExecutionScreen(RedunScreen):
    ...
    def action_focus(self) -> None:  # ty: ignore[invalid-method-override]
        """
    ...
    async def load_jobs(self):
        ...
        if not self.args.job:
            ...
            self.jobs = []  # ty: ignore[invalid-assignment]
        ...
        self.job_list.jobs = self.jobs  # ty: ignore[invalid-assignment]
...
class ExecutionsScreen(RedunScreen):
    ...
    def watch_args(self) -> None:
        ...
        # Reload executions.
        self.execution_list.executions = None  # ty: ignore[invalid-assignment]
