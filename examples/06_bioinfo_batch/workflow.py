...
FASTQ_PATTERN = ".*/.*/(?P<sample>.*)_(?P<flowcell>.*)_(?P<lane>.*)_(?P<read>.*).fastq.gz"
...
genome_ref_src = File("https://hgdownload.soe.ucsc.edu/goldenPath/hg38/chromosomes/chr22.fa.gz")
...
def index_genome_ref(genome_ref_file: File) -> Dict[str, File]:
    ...
    return script(
        ...
        outputs={key: File(f"{base_path}.{ext}").stage() for key, ext in zip(keys, index_exts)},
    )
...
def download_sites_files(
    ...
    sites_files = [
        copy_file(file, os.path.join(ref_dir, file.basename()), skip_if_exists=skip_if_exists)
...
def group_sample_read_pairs(
    ...
    """
    sample_read_files: Dict[str, Dict[str, List[str]]] = defaultdict(lambda: defaultdict(list))
...
@task(version="1")
def get_sample_read_pairs(read_dirs: List[str]) -> Dict[str, Dict[str, List[str]]]:
    """
...
@task(version="1")
def merge_fastq(reads1: List[File], reads2: List[File], output_path: str) -> Dict[str, File]:
    """
...
def cut_adapters(
    ...
    return script(
        ...
        outputs={
            ...
            "reads1": File(f"{output_path}.1.fastq.gz").stage("cutadapt.1.fastq.gz"),
            "reads2": File(f"{output_path}.2.fastq.gz").stage("cutadapt.2.fastq.gz"),
        },
    )
...
def base_recalib(
    ...
    known_sites_args = " ".join(f"--known-sites {f.local.path}" for f in sites_stage_files)
    ...
    return script(
        ...
        outputs={
            ...
            "bai": File(f"{output_path}.bam.bai").stage("sample.processed.bam.bai"),
...
def collect_align_metrics(
    ...
    result = script(
        ...
        outputs={
            ...
            "insert_metrics.status": File(f"{output_path}.insert_metrics.status").stage(
                "sample.insert_metrics.status"
            ),
            "insert_metrics": File(f"{output_path}.insert_metrics.txt").stage(
                "sample.insert_metrics.txt"
            ),
            "insert_size_hist": File(f"{output_path}.insert_size_histogram.pdf").stage(
...
def collect_depth_metrics(bam: File, output_path: str) -> Dict[str, File]:
    ...
    return script(
        ...
        outputs={
            "depth": File(f"{output_path}.depth.txt").stage("sample.depth_out.txt"),
        },
    )
...
def prepare_samples(output_path: str) -> File:
    ...
    samples_file = write_samples_file(data_dir.files(), os.path.join(output_path, "samples.txt"))
    return samples_file
